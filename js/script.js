...
async function processUserInput() {
    ...
    // loadingIndicator.classList.add('loading-indicator'); // CSS에서 이 클래스로 꾸미기
...
function saveChatHistory() {
    ...
    chatBox.querySelectorAll('p').forEach(p => {
        ...
        // 메시지 내용을 가져오기 (HTML 태그 제거하고 텍스트만)
        // innerText 사용 시 줄바꿈 등 서식 유지에 좋음
        const text = p.innerText; // 또는 p.textContent;
...
function addMessageToChat(sender, message) {
    ...
    if (sender === 'user') {
        ...
        // 메시지 내용에 '나:' 같은 이름은 CSS에서 숨기므로 실제 내용만 넣어요.
...
if (initialMessageElement && !initialMessageElement.classList.contains('ai-message')) {
